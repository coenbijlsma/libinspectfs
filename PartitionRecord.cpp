...
#include <stdlib.h>
#include <string.h>
const unsigned short PartitionRecord::STATUS_BOOTABLE;
const unsigned short PartitionRecord::STATUS_NON_BOOTABLE;

PartitionRecord::PartitionRecord( MBR* mbr, char* bytes ) throw( Exception )
        : _mbr( mbr ) {
    if ( bytes == 0 ) {
    ...
    this->_status = copy[ 0x00 ];


    this->_chsFirst[ 0 ] = (int)copy[ 0x01 ]; // head
    this->_chsFirst[ 1 ] = (int)copy[ 0x02 ]; // sector & cylinder
    this->_chsFirst[ 2 ] = (int)copy[ 0x03 ]; // cylinder

    this->_type = copy[ 0x04 ];

    this->_chsLast[ 0 ] = (int)copy[ 0x05 ]; // head
    this->_chsLast[ 1 ] = (int)copy[ 0x06 ]; // sector & cylinder
    this->_chsLast[ 2 ] = (int)copy[ 0x07 ]; // cylinder

    this->_lbaFirst[ 3 ] = copy[ 0x08 ];
    this->_lbaFirst[ 2 ] = copy[ 0x09 ];
    this->_lbaFirst[ 1 ] = copy[ 0x0a ];
    this->_lbaFirst[ 0 ] = copy[ 0x0b ];

    // little endian
    this->_sectorCount[ 3 ] = copy[ 0x0c ];
    this->_sectorCount[ 2 ] = copy[ 0x0d ];
    this->_sectorCount[ 1 ] = copy[ 0x0e ];
    this->_sectorCount[ 0 ] = copy[ 0x0f ];
    delete copy;
}
...
bool PartitionRecord::isBootable() {
    return STATUS_BOOTABLE == this->_status;
}

unsigned char PartitionRecord::getType() {
    return this->_type;
}

unsigned short PartitionRecord::getFirstHead() {
    return (unsigned short) this->_chsFirst[ 0 ];
}

unsigned short PartitionRecord::getFirstSector() {
    return (unsigned short) ( this->_chsFirst[ 1 ] & 0x3f );
}

unsigned short PartitionRecord::getFirstCylinder() {
    return (unsigned short)( ( this->_chsFirst[ 1 ] & 0xc0 ) << 2 ) + this->_chsFirst[ 2 ];
}

unsigned short PartitionRecord::getLastHead() {
    return (unsigned short)this->_chsLast[ 0 ];
}

unsigned short PartitionRecord::getLastSector() {
    return (unsigned short) ( this->_chsLast[ 1 ] & 0x3f );
}

unsigned short PartitionRecord::getLastCylinder() {
    return (unsigned short)( ( this->_chsLast[ 1 ] & 0xc0 ) << 2 ) + this->_chsLast[ 2 ];
}

unsigned long PartitionRecord::getLBAFirst() {
    unsigned long lba = 0;

    // todo move this to constructor.
    lba |= (unsigned char)this->_lbaFirst[ 0 ];

    lba <<= 8;
    lba |= (unsigned char)this->_lbaFirst[ 1 ];

    lba <<= 8;
    lba |= (unsigned char)this->_lbaFirst[ 2 ];

    lba <<= 8;
    lba |= (unsigned char)this->_lbaFirst[ 3 ];

    return lba;
}

unsigned long PartitionRecord::getSectorCount() {
    unsigned long count = 0;

    // todo move this to constructor.
    count |= (unsigned char)this->_sectorCount[ 0 ];

    count <<= 8;
    count |= (unsigned char)this->_sectorCount[ 1 ];

    count <<= 8;
    count |= (unsigned char)this->_sectorCount[ 2 ];

    count <<= 8;
    count |= (unsigned char)this->_sectorCount[ 3 ];

    return count;
}

bool PartitionRecord::isExtended() {
    switch( this->_type ) {
        case 0x05:
        case 0x0f:
        case 0x3b:
...
MBR* PartitionRecord::getMBR() {
    return this->_mbr;
}

string PartitionRecord::getTypeDescription(unsigned char type) {
